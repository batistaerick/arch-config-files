...
choose_ecr_repository() {
  ...
  chosen="$(printf "%s\n" "$repositories" | walker_menu "ECR Repository")"
...
chosen=$(echo -e "$options" | $HOME/.config/walker/bin/walker-dmenu --dmenu --no-sort --matching=contains --cache-file /dev/null --prompt="ECR - $AWS_PROFILE")
