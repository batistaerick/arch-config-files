...
choose_iam_user() {
  ...
  chosen="$(printf "%s\n" "$users" | walker_menu "IAM User")"
  [ -z "$chosen" ] && exit 0
  echo "$chosen"
}

options="󰀄  Account summary
󰀄  Users
󰌾  Roles
󰒃  Policies
󰘦  Groups
...
chosen=$(echo -e "$options" | $HOME/.config/walker/bin/walker-dmenu --dmenu --no-sort --matching=contains --cache-file /dev/null --prompt="IAM - $AWS_PROFILE")
