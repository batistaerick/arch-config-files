...
run_ai_tool() {
  ...
  repo="$(
    find "$dev_dir" -mindepth 3 -maxdepth 3 -type d -name ".git" |
      sed "s|$dev_dir/||; s|/.git||" |
      sort |
      $HOME/.config/walker/bin/walker-dmenu --width 500 --dmenu --no-sort --cache-file /dev/null --prompt="Repository"
  )"
...
prompt_text() {
  ...
  printf "" |
    $HOME/.config/walker/bin/walker-dmenu --width 500 --dmenu --no-sort --cache-file /dev/null --prompt="$prompt"
}
...
chosen="$(
  echo "$options" |
    sed '/^[[:space:]]*$/d' |
    $HOME/.config/walker/bin/walker-dmenu --width 500 --dmenu --no-sort --matching=multi-contains --cache-file /dev/null --prompt="Search All" --search "$initial_query"
)"
