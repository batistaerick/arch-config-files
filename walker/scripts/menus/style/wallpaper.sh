...
chosen="$(
  find "$BACKGROUND_DIR" -maxdepth 1 -type f \
    ...
    -printf "%f\n" |
    sort -V |
    $HOME/.config/walker/bin/walker-dmenu --dmenu --no-sort --cache-file /dev/null --prompt="Wallpaper"
)"
...
confirm="$(
  printf "Apply\nCancel\n" |
    $HOME/.config/walker/bin/walker-dmenu --dmenu --no-sort --cache-file /dev/null --prompt="Apply this wallpaper?"
)"
