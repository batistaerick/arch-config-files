...
chosen="$(
  theme_options |
    wofi --dmenu --no-sort --allow-markup --parse-search --cache-file /dev/null --prompt="Theme"
)"
...
Cancel"
  ...
  action="$(
    printf '%s\n' "$options" |
    wofi --dmenu --no-sort --cache-file /dev/null --prompt="Apply $theme_name?"
  )"

  case "$action" in
    "Apply")
      close_preview
      setsid -f "$HOME/.config/wofi/scripts/actions/style/apply.sh" "$theme_name" >/tmp/apply.log 2>&1 < /dev/null
      exit 0
      ;;
    "←  Back")
      close_preview
